...
-- | Convert a possibly-incomplete specification for what to convert into one which can be
-- executed. Calls error when data is missing.
fromJustConvertSpec :: ConvertSpec Maybe -> ConvertSpec Identity
fromJustConvertSpec convertSpec =
  convertSpec
...
mergeArg (ConvertLhsStyle lhsStyle) convertSpec
  ...
  = error $
    printf "Conflicting lhs styles requested: <%s> and <%s>" (show lhsStyle) (show previousLhsStyle)
...
mergeArg unexpectedArg _ =
  error $ "IHaskell.Convert.mergeArg: impossible argument: " ++ show unexpectedArg
